...
export default function ArtistDetail() {
  ...
  const { artist: { name = "", images = [], genres } = {} } = useLocation()
    ?.state as { artist: Artist };
  const t = [
    {
      album: {
        album_type: "single",
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-2 my-2">
        ...
        <div className="col-span-2">
          <Routes>
            <Route
              path={`${path}/top-tracks`}
              children={<Tracks tracks={t} />}
            />
            <Route
              path={`${path}/related-artists`}
              children={<Artists artists={a} />}
            />
          </Routes>
        </div>
      </div>
